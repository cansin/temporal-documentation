...
In the following example, a single Event History is loaded from a JSON file on disk (as obtained from the Web UI](/web-ui) or the [Temporal CLI:
...
You can do so by combining the Client.workflow.list() and [worker.runReplayHistories()](https://typescript.temporal.io/api/classes/worker.Worker#runreplayhistories) APIs.
