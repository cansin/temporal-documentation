...
Running a Temporal Service locally, like through the Temporal CLI, requires minimal connection configurations.
...
To connect to the Temporal Service, you create a Connection. You then use this Connection when setting up the Client. You can pass the `Connection` instance when creating the Client.
...
See the [Customize Workflow Type](/develop/typescript/core-application#workflow-type) section to see how to customize the name of the Workflow Type.
